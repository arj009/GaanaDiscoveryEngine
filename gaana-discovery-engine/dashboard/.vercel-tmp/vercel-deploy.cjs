...
function runBuildIfNeeded(projectPath) {
  ...
  if (!packageJson.scripts || !packageJson.scripts.build) { log('No build script found'); return true; }
  ...
  try {
    ...
    log('Build completed successfully!'); return true;
  } catch (error) {
    log('Build FAILED!'); process.exit(1);
  }
}
